    ...
    <div class="col-12">
        <div class="card-box table-responsive">
            <div class="row mb-2">
                ...
                <div class="col-2">
                    <a href="{{route('admin.subcategory_form.add')}}" class="btn btn-success btn-xs"><i class="fa fa-plus"></i> Add {{$delete_title}}</a>
                </div>
            </div>
            <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
              <thead>
                ...
                <tbody>
                 @foreach($subcategory as $key => $enq)
                    <tr>
                        <td>{{++$key}}</td>
                        <td>
                            <img src="{{asset('public/uploads/category/'.$enq->image)}}" 
                            class="img-responsive w-25">
                        </td>
                        <td>{{getCategory($enq->parent_id)->title?? ''}}</td>
                        <td>{{$enq->title}}</td>
                        <td>
                        ...
                        <td>
                            <a href="{{url('admin/subcategory-edit/'.$enq->id)}}" class="btn btn-xs bg-primary text-white">
