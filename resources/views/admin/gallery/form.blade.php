                 ...
                 <form class="form-horizontal" action="{{ route('admin.gallery.store')}}" method="post" id="addForm"   enctype="multipart/form-data">
                    ...
                    <input type="hidden" name="id" value="{{isset($gallery)?$gallery->id:''}}">
                                ...
                                <label class="col-md-2 col-form-label" for="example-email">Image</label>
                                    ...
                                    <div class="col-md-4">
                                        <img src="{{isset($gallery)?asset('public/uploads/gallery/'.$gallery->image):''}}" class="img-responsive w-25">
                                    </div>
                                </div>
                                {{------
                               @if(!empty($service))
                                        ...
                                        <select class="form-control" name="service_id">
                                            <option value="" selected disabled>Choose Service</option>
                                             @foreach($service as $ser)
                                                <option value="{{$ser->id}}" {{ isset($gallery)?($gallery->service_id==$ser->id?'selected':''):''}}>{{$ser->title}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-10">
                                        <select class="form-control" name="type">
                                            ...
                                            <option value="press" {{ isset($gallery)?($gallery->type=='press'?'selected':''):''}}>Press Release</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="form-group row">
