    ...
    <div class="col-12">
        <div class="card-box table-responsive">
            <div class="row mb-2">
                ...
                <div class="col-2">
                    <a href="{{route('admin.course.add')}}" class="btn btn-success btn-xs"><i class="fa fa-plus"></i> Add {{$delete_title}}</a>
                </div>
            </div>
            <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                ...
                <tbody>
                 @foreach($course as $key => $enq)
                    <tr>
                        <td>{{++$key}}</td>
                        <td>{{substr($enq->title,0,35)}}</td>
                        <td>{{$enq->duration}}</td>
                        {{-----
                        <td><img src="{{asset('public/uploads/course/'.$enq->image)}}" class="w-25"></td>

                        <td>{{ $enq->category?getCategory($enq->category)->title:''}}</td>
                        ----}}
                        <td>
                        ...
                        <td>
                            <a href="{{url('admin/course/edit/'.$enq->id)}}" class="btn btn-xs bg-primary text-white">
