    ...
    <div class="col-12">
        <div class="card-box table-responsive">
            <div class="row mb-2">
                ...
                <div class="col-2">
                    <a href="{{route('admin.enquiry_form.add')}}" class="btn btn-success btn-xs"><i class="fa fa-plus"></i> Add {{$delete_title}}</a>
                </div>
            </div>
                ...
                <tbody>
                 @foreach($enquiry as $key => $enq)
                    <tr>
                        ...
                        <td>{{getService($enq->service)->title}}</td>
                        <td>
                            {{$enq->adults}} 
                        </td>
                        <td>{{show_date($enq->created_at)}}</td>
                        <td>
                            <a href="{{url('admin/enquiry-edit/'.$enq->id)}}" class="btn btn-xs bg-primary text-white">
